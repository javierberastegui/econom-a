...
class Routes {
	...
	public function register(): void {
		register_rest_route( self::NAMESPACE, '/accounts', array(
			...
			array( 'methods' => 'POST', 'callback' => array( $this, 'create_account' ), 'permission_callback' => fn() => $this->can( Capabilities::MANAGE_ACCOUNTS ) ),
		) );
		register_rest_route( self::NAMESPACE, '/accounts/(?P<id>\d+)', array(
			array( 'methods' => 'GET', 'callback' => array( $this, 'get_account' ), 'permission_callback' => fn() => $this->can( Capabilities::MANAGE_ACCOUNTS ) ),
			array( 'methods' => 'PUT', 'callback' => array( $this, 'update_account' ), 'permission_callback' => fn() => $this->can( Capabilities::MANAGE_ACCOUNTS ) ),
			array( 'methods' => 'DELETE', 'callback' => array( $this, 'delete_account' ), 'permission_callback' => fn() => $this->can( Capabilities::MANAGE_ACCOUNTS ) ),
		) );
